...
namespace cl
{
	...
	public:
		...
		{
			Node* to_del = it.node_;

			cnt_--;

			Node* del_child = nullptr;
			if (to_del->left_ == nullptr)
				del_child = to_del->right_;
			else
			{
				if (to_del->right_ == nullptr)
					del_child = to_del->left_;
				else
				{
					//查找删除替代节点
					Node* tmp = to_del->right_;
					while (tmp->left_) tmp = tmp->left_;
					del_child = tmp->right_;
					if (tmp != to_del)
					{
						//node的key和val不用管, 因为要删除node
						to_del->key() = std::move(tmp->key());
						to_del->val() = std::move(tmp->val());
						to_del = tmp;
					}
				}
			}

			Node* parent = to_del->parent_;
			if (del_child) del_child->parent_ = parent;
			if (parent)
			{
				if (parent->left_ == to_del)
					parent->left_ = del_child;
				else
					parent->right_ = del_child;
			}
			else
				root_ = del_child;

			if (to_del->is_black()) _remove_repair(parent, del_child);
			free_obj<MA>(to_del);
		}
