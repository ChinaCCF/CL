	...
	namespace lib
	{
		...
		public:
			...
			{ 
				T buf[64];
				uv32 len = cstr::from_val(buf, 64, val);
				_NString ret; ret.append(buf, len);
				return ret;
			} 
			template<FloatType F>
			_NString operator+(const Fraction<F>& f)
			...
			template<CharType T2>
			_NString operator+(const T2* str)
			{
				_NString ret; ret << str;
				return ret;
			}

			template<CharType T2>
			_NString operator+(const _StrView<T2>& str)
			{
				_NString ret; ret << str;
				return ret;
			}
			...
			template<CharType T2, MemAllocType MA2, uv32 M>
			_NString operator+(const _NString<T2, MA2, M>& str)
			{
				_NString ret; ret << str;
				return ret;
			} 
